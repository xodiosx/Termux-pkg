...
termux_step_pre_configure() {
	...
	# Patch configure script to use Termux's ALSA paths
	sed -i \
		-e 's|/usr/include/alsa|'"$TERMUX_PREFIX"'/include/alsa|g' \
		-e 's|/usr/local/include/alsa|'"$TERMUX_PREFIX"'/include/alsa|g' \
		-e 's|/usr/lib|'"$TERMUX_PREFIX"'/lib|g' \
		configure
...
termux_step_configure() {
	# Run configure manually – omits the unwanted --disable-dependency-tracking
	./configure \
